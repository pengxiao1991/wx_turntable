...
var Turntable = (function (_super) {
    ...
    function Turntable() {
        ...
        _this.scrollCount = 0;
        _this.open = false; //判断是否开奖
        _this.isBet = false; //用户是否投注
        _this.lastOpenId = 0; //上次开奖ID
        _this.IN_COUNT_DOWN_COUNT = 0; //进入游戏时截盘偏移量
        _this.SCREEN_IS_OFF = false; //是否息屏
        _this.addEventListener(eui.UIEvent.COMPLETE, _this.uiCompHandler, _this);
        _this.skinName = 'resource/customSkins/turntableLayout/turntable.exml';
        return _this;
    }
    Turntable.prototype.uiCompHandler = function () {
     ...
     * 倒计时结束
     */
    Turntable.prototype.countDownEnd = function (res, rej, data) {
        this.currentState = 'opening'; //设置当前状态为开奖中
        this.startScroll(res, rej, data);
        this.getOpenAwrad();
    };
    //转盘开始动画
    Turntable.prototype.startScroll = function (res, rej, data) {
        var self = this;
        this.stage.frameRate = 60; //修改游戏帧率
        var tw = self.stopScrollTableTween();
        tw.to({ rotation: this.scrollTable.rotation + 360 }, 2000, egret.Ease.sineIn).call(function () {
            self.middSrcollAnima(res, rej, data);
        });
        self.startLightAnmi();
    };
    //转盘中间持续动画
    Turntable.prototype.middSrcollAnima = function (res, rej, data) {
        var self = this, angle = 0, //结束转盘动画时需要转动多少格子
        keepTime = 4 + data > 1 ? 4 + data : 1; //持续时间
        self.scrollCount += 1;
        egret.Tween.removeTweens(this.scrollTable);
        var tw = self.stopScrollTableTween();
        tw.to({ rotation: this.scrollTable.rotation + 360 }, 1000).call(function () {
            if (self.scrollCount > keepTime && openInfo.isGetCurrentRouletteResult && openInfo.isGetMyOpenInfo) {
                /**
                 * 1.根据中奖盘点ID减去上一次开奖判断ID来计算需要偏转的角度（angle）
                 * 2.当角度小于180度时 angle += 360  当角度大于180度时 angle不变
                 * 3.把持续转动圈数设为0;
                */
                var currentPanId = openInfo.number; //当前开奖ID
                /**
    ...
    //结束转盘动画
    Turntable.prototype.endScrollAnima = function (angle, res, rej) {
        var _this = this;
        var self = this, tw = self.stopScrollTableTween(), time = 2500 * angle / 27;
        tw.to({ rotation: this.scrollTable.rotation + 20 * angle / 3 }, time, egret.Ease.circOut).call(function () {
            // 更新历史记录			
            GlobalEmitter.emit(GlobalEvent.UPDATEHISTORY, openInfo.result.hits[0], CURRENT_NO);
            res(); //转盘动画结束开启开奖结果
            _this.stage.frameRate = 40; //修改游戏帧率
        });
    };
    //拉取开奖结果
    Turntable.prototype.getOpenAwrad = function () {
        var self = this;
        self.open = true;
        //用户投注 查询开奖结果
        if (!this.isBet) {
    ...
    //查询用户中奖结果回调
    Turntable.prototype.getUserStakeResultCallback = function (response) {
    ...
    //已经拿到本期中奖信息
    Turntable.prototype.getCurrentLotteryResultCallback = function (response) {
    ...
    //开奖
    Turntable.prototype.lottery = function (res, rej) {
        var self = this;
        var _time = setTimeout(function () {
    ...
    //停止转盘上的动画，并创建一个新的动画
    Turntable.prototype.stopScrollTableTween = function () {
        egret.Tween.removeTweens(this.scrollTable);
        return egret.Tween.get(this.scrollTable);
    };
    //计算中奖结果
    Turntable.prototype.countLotteryResult = function () {
        GlobalEmitter.emit(GlobalEvent.START_OPEN_AWARD, this.isBet, openInfo);
    };
    //新游戏开始
    Turntable.prototype.newGameOpen = function (data) {
    ...
    //开启霓虹灯动画
    Turntable.prototype.startLightAnmi = function () {
    ...
    //关闭霓虹灯动画
    Turntable.prototype.endLightAnmi = function () {
        egret.Tween.removeTweens(this.light);
        egret.Tween.removeTweens(this.light1);
    };
    //画一个开奖时蒙层
    Turntable.prototype.drawC = function () {
    ...
    //息屏时移除节点上的动画
    Turntable.prototype.screenOff = function () {
        egret.Tween.removeTweens(this.scrollTable);
        egret.Tween.removeTweens(this.light);
        egret.Tween.removeTweens(this.light1);
...
__reflect(Turntable.prototype, "Turntable");
